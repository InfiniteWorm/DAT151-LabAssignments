...
checkArgs args = mapM_ (\ (ADecl t x) -> newVarType x t) args
...
lookupVarType :: Id -> [Block] -> TC Type
lookupVarType id [] = throwError $ "variable undefined : " ++ printTree id
lookupVarType id block = do
  let headbl = head(block)
  case Map.lookup id headbl of
    Nothing -> do
      let tailbl = tail(block)
      lookupVarType id tailbl
    Just t -> return t
